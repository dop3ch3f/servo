    ...
    fn build_fragment_type_specific_display_items(
        ...
        let create_base_display_item = |state: &mut DisplayListBuildState| {
            ...
            if !radii.is_zero() {
                ...
                let border_widths =
                    self.style.logical_border_width().to_physical(self.style.writing_mode);
                let clip_id =
                    state.add_late_clip_node(stacking_relative_border_box.inner_rect(border_widths).to_layout(), radii);
                state.current_clipping_and_scrolling = ClippingAndScrolling::simple(clip_id);
            }
